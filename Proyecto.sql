-- Crear esquema KeepCoding
CREATE SCHEMA KeepCoding;

-- Crear la tabla Grupo Empresarial
...
CREATE TABLE KeepCoding.Marca (
    ...
    FOREIGN KEY (id_grupo_empresarial) REFERENCES KeepCoding.Grupo_Empresarial(id_grupo_empresarial)
);
...
CREATE TABLE KeepCoding.Vehiculo (
    ...
    kms_totales NUMERIC(10,2) NOT NULL,
    FOREIGN KEY (id_marca) REFERENCES KeepCoding.Marca(id_marca)
);
...
CREATE TABLE KeepCoding.Seguros (
    ...
    n_poliza VARCHAR(20) NOT NULL,
    fecha_alta_seguro DATE NOT NULL DEFAULT now(),
    matricula VARCHAR(9) NOT NULL,
    FOREIGN KEY (id_aseguradora) REFERENCES KeepCoding.Aseguradora(id_aseguradora),
    FOREIGN KEY (matricula) REFERENCES KeepCoding.Vehiculo(matricula)
);

-- Crear la tabla Revision_Vehiculo
CREATE TABLE KeepCoding.Revision_Vehiculo (
    id_revision SERIAL PRIMARY KEY,
    importe_revision NUMERIC(10,2) NOT NULL,
    moneda VARCHAR(20) NOT NULL,
    fecha_revision DATE NOT NULL DEFAULT now(),
    kms_revision NUMERIC(10,2) NOT NULL,
    matricula VARCHAR(9) NOT NULL,
    FOREIGN KEY (matricula) REFERENCES KeepCoding.Vehiculo(matricula)
);
...
--INSERTAR DATOS
-- Insertar datos en Grupo_Empresarial
INSERT INTO KeepCoding.Grupo_Empresarial (nombre_grupo_empresarial)
SELECT DISTINCT grupo
FROM coches;

-- Insertar datos en Aseguradora
INSERT INTO KeepCoding.Aseguradora (nombre_aseguradora)
SELECT DISTINCT aseguradora
FROM coches;

-- Insertar datos en Marca
INSERT INTO KeepCoding.Marca (nombre_marca, id_grupo_empresarial)
SELECT DISTINCT coches.marca, grupo.id_grupo_empresarial
FROM coches
JOIN KeepCoding.Grupo_Empresarial grupo ON coches.grupo = grupo.nombre_grupo_empresarial;

-- Insertar datos en Vehiculo evitando duplicados
INSERT INTO KeepCoding.Vehiculo (matricula, id_marca, modelo, color, fecha_compra, kms_totales)
SELECT matricula, marca.id_marca, modelo, color, fecha_compra, kms_totales
FROM coches
JOIN KeepCoding.Marca marca ON coches.marca = marca.nombre_marca
ON CONFLICT (matricula) DO NOTHING;

-- Insertar datos en Seguros
INSERT INTO KeepCoding.Seguros (id_aseguradora, n_poliza, fecha_alta_seguro, matricula)
SELECT
    (SELECT id_aseguradora FROM KeepCoding.Aseguradora WHERE nombre_aseguradora = coches.aseguradora),
    n_poliza,
    fecha_alta_seguro,
    matricula
FROM coches;

-- Insertar datos en Revision_Vehiculo
INSERT INTO KeepCoding.Revision_Vehiculo (importe_revision, moneda, fecha_revision, kms_revision, matricula)
SELECT
    importe_revision,
    moneda,
    fecha_revision,
...
SELECT
    ...
    v.modelo AS Modelo,
    g.nombre_grupo_empresarial AS Grupo,
    v.fecha_compra AS "Fecha de Compra",
    v.matricula AS Matricula,
    v.color AS Color,
    v.kms_totales AS "Total de Kilometros",
    a.nombre_aseguradora AS "Nombre de la Empresa Aseguradora",
    s.n_poliza AS "Numero de Poliza"
FROM KeepCoding.Vehiculo v
INNER JOIN KeepCoding.Marca m ON v.id_marca = m.id_marca
INNER JOIN KeepCoding.Grupo_Empresarial g ON m.id_grupo_empresarial = g.id_grupo_empresarial
LEFT JOIN KeepCoding.Seguros s ON v.matricula = s.matricula
LEFT JOIN KeepCoding.Aseguradora a ON s.id_aseguradora = a.id_aseguradora;
